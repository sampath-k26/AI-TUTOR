...
- Frontend: React (Vite) + TypeScript, styled with Tailwind CSS v3 + a small shadcn/ui-style component set (`src/components/ui/`: button, card, input, textarea, label, badge). Design tokens (HSL CSS variables in `src/index.css`, mapped in `tailwind.config.js`) are adapted from a reference "Linear-grade" design language (near-black canvas, four-step surface ladder, hairline borders instead of shadows, a single lavender-blue accent `#5e6ad2`) — read `~/Documents/Zopkit/Project-Management-master` for the original if extending this further, but never write to that repo. Theme is a manual light/dark toggle (`darkMode: 'class'`, `.dark` on `<html>`, state in `lib/ThemeContext.tsx`, defaults to OS preference on first visit) — an inline script in `index.html` applies the stored/system theme before first paint to avoid a flash of the wrong theme; keep the two in sync if the storage key or logic changes.
- Authenticated pages get their chrome from `components/layout/AppShell.tsx` (a persistent sidebar via `components/layout/Sidebar.tsx` + a content `<Outlet/>`), mounted once as a layout route in `App.tsx` wrapped by `ProtectedRoute` — individual pages don't render their own header nav/logout/theme-toggle; only page-specific content and, where genuinely hierarchical (e.g. Project → its Space), a breadcrumb-style back-link. The sidebar itself is pure navigation only (Spaces, Global Analytics, Projects, Activity Log, plus Admin for admins) via a single `PrimaryNav` component — no search/filter/embedded lists live in it. Anything that needs search, filters, tabs, or pagination (the Projects list, the Projects/Spaces Activity Log) is its own full-page route (`/projects`, `/activity`) that fetches fresh on mount, not a sidebar sub-section — a prior design embedded that content directly into the sidebar and had to be corrected. Because those pages always refetch on navigation, no cross-component "refresh the sidebar after this mutation" mechanism is needed.
- Create-a-`<Thing>` forms open via a button into `components/ui/sheet.tsx` (a right-side peek panel on desktop, centered dialog on small screens), not inlined in the page layout — see `HomePage`/`SpaceDetailPage` for the pattern (Sheet + form + `useToast()` on success/failure). List views (Spaces, Projects) render as a card grid, whole card clickable via `Link` wrapping `Card interactive`, not a title-only link.
- Every CRUD action gets a toast (`useToast()` from `lib/ToastContext.tsx`, rendered by the single `<Toaster/>` mounted in `App.tsx`) — success and failure both, not just failure.
- Loading states are `components/ui/skeleton.tsx` (or `table-skeleton.tsx` for tabular data) shaped like the content about to appear — never bare "Loading…" text.
