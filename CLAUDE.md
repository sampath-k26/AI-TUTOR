...
- Authenticated pages get their chrome from `components/layout/AppShell.tsx` (a persistent sidebar via `components/layout/Sidebar.tsx` + a content `<Outlet/>`), mounted once as a layout route in `App.tsx` wrapped by `ProtectedRoute` — individual pages don't render their own header nav/logout/theme-toggle; only page-specific content and, where genuinely hierarchical (e.g. Project → its Space), a breadcrumb-style back-link. The sidebar itself has two independently-fetching sub-sections (`SidebarProjectsSection`, `SidebarActivitySection`) that persist across route changes — any mutation that should be reflected there (creating a Space/Project, uploading a material, ...) must call `useSidebarRefresh().refreshSidebar()` after success, or the sidebar goes stale until a full reload (a real bug found live before this was added).
- Create-a-`<Thing>` forms open via a button into `components/ui/sheet.tsx` (a right-side peek panel on desktop, centered dialog on small screens), not inlined in the page layout — see `HomePage`/`SpaceDetailPage` for the pattern (Sheet + form + `useToast()` on success/failure + `refreshSidebar()` if the created thing affects the sidebar). List views (Spaces, Projects) render as a card grid, whole card clickable via `Link` wrapping `Card interactive`, not a title-only link.
