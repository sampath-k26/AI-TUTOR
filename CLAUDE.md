...
- Real Supabase credentials now exist: project `wuhvesnhkjgstsbpkeyn` ("aitutor", region ap-northeast-2), created 2026-09-16. `apps/api/.env` and `apps/web/.env` are filled in with `DATABASE_URL` (transaction pooler), `SUPABASE_URL`, `SUPABASE_ANON_KEY`. Still missing: `SUPABASE_SERVICE_ROLE_KEY` (needed for Storage uploads/downloads) and `GEMINI_API_KEY`/`GROQ_API_KEY` — ask the user for these when a task needs them (Storage, or any real AI call) rather than guessing or leaving them silently broken.
...
- New Supabase projects require email confirmation before `signUp()` returns a session by default. For a one-off manual/smoke test needing a real JWT, sign up, then confirm via `apply_migration`: `UPDATE auth.users SET email_confirmed_at = now() WHERE id = '<id>'`, then `signInWithPassword()`. Clean up any test users/rows created this way afterward (delete from `public.*` tables first, then `auth.users`, respecting FKs) — don't leave smoke-test data in the user's real project.
